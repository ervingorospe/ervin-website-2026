...
const socialMedias = [
  {
    link: "https://www.facebook.com/ErvinN09/",
    icon: (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 320 512"
        className="h-6 w-6 md:h-8 md:w-8 text-primary group-hover:text-background"
      >
        <path
          fill="currentColor"
  ...
  {
    link: "https://github.com/ervingorospe",
    icon: (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 512 512"
  ...
  return (
    <>
      {socialMedias?.map((social) => (
        <RingHoverButton key={social.link}>
          <Link href={social.link} target="_blank">
            <Paper className="relative z-10 hover:bg-primary p-2 border border-primary rounded-full bg-paper transition-colors duration-300 ease-in-out">
              {social.icon}
            </Paper>
          </Link>
        </RingHoverButton>
